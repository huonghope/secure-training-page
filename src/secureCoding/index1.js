...
class Securecoding extends React.Component {
  ...
  handleDiff = () => {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
      ...
      // step2GetLine = monaco.editor.getModels()[1].getFullModelRange().endLineNumber


      //     setTimeout(function () {

      //     //선택한 text 범위의 시작과 끝 줄이 정답라인이 아닐경우  readonly:true

      //     diffEditor.getModifiedEditor().onMouseDown(e => {
      //       //let line = e.target.position.lineNumber;
      //       console.log(diffEditor.getModifiedEditor().getSelection())
      //       console.log("startline : " + diffEditor.getModifiedEditor().getSelection().startLineNumber)
      //       console.log("endline : " + diffEditor.getModifiedEditor().getSelection().endLineNumber)


      //       if (myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getSelection().startLineNumber) !== -1 && 
      //       myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getSelection().endLineNumber) !== -1) {
      ...
      //       // if (diffEditor.getModifiedEditor().getPosition().lineNumber != 5) { diffEditor.getModifiedEditor().updateOptions({ readOnly: true }); }
      //       // else {
      //       //   diffEditor.getModifiedEditor().updateOptions({ readOnly: false });
      //       // }
      //     });
      ...
      //       label: 'My Label!!!',
      //       keybindings: [
      //         // monaco.KeyMod.CtrlCmd | monaco.KeyCode.Enter, monaco.KeyMod.Shift | monaco.KeyCode.Enter, monaco.KeyCode.Insert, monaco.KeyMod.CtrlCmd | monaco.KeyCode.KEY_V,
      //         // monaco.KeyMod.CtrlCmd | monaco.KeyCode.KEY_X, monaco.KeyCode.Enter, monaco.KeyMod.CtrlCmd | monaco.KeyCode.Backspace, monaco.KeyMod.Shift | monaco.KeyCode.Backspace,
      //         // monaco.KeyCode.Delete, monaco.KeyMod.CtrlCmd | monaco.KeyCode.Delete, monaco.KeyMod.Shift | monaco.KeyCode.Delete, monaco.KeyMod.Shift | monaco.KeyCode.Insert,
      //         // monaco.KeyMod.Shift | monaco.KeyMod.CtrlCmd | monaco.KeyCode.Enter,
      // // 2 초기화 할 때 정답 라인도 초기화
      //         // 엔터만 품
      //         monaco.KeyCode.Insert, monaco.KeyMod.CtrlCmd | monaco.KeyCode.KEY_V, //monaco.KeyCode.Delete, monaco.KeyMod.CtrlCmd | monaco.KeyCode.Delete, monaco.KeyMod.Shift | monaco.KeyCode.Delete,
      //         monaco.KeyMod.CtrlCmd | monaco.KeyCode.KEY_X, monaco.KeyMod.CtrlCmd | monaco.KeyCode.Backspace, monaco.KeyMod.Shift | monaco.KeyCode.Backspace,
      ...
      //     //여기서부터
      //     var myCondition1 = diffEditor.getModifiedEditor().createContextKey(/*key name*/'myCondition1', /*default value*/false);
      //     var myCondition2 = diffEditor.getModifiedEditor().createContextKey(/*key name*/'myCondition2', /*default value*/false);

      //     diffEditor.getModifiedEditor().addCommand(monaco.KeyCode.Backspace, function () {

      //       // 마우스로 정답라인 뒤에서 맨앞까지 범위 지정했을때 백스페이스 누르면 지정범위 내용 없애줌
      //       var op = { identifier: "1", range: diffEditor.getModifiedEditor().getSelection(), text: "", forceMoveMarkers: true };
      //       diffEditor.getModifiedEditor().executeEdits("my-source", [op]);

      //       return;
      //     }, 'myCondition1')


      //     // diffEditor.getModifiedEditor().addCommand(monaco.KeyCode.Enter, function () {

      //     //   diffEditor.getModifiedEditor().trigger('', 'acceptSelectedSuggestion')
      //     //   // diffEditor.getModifiedEditor().trigger('','type',{ text: '\n' })
      //     //   return ;
      //     // }, 'myCondition2')

      //     diffEditor.getModifiedEditor().addCommand(monaco.KeyCode.Delete, function () {
      ...
      //       if(diffEditor.getModifiedEditor().getModel().getFullModelRange().endLineNumber > step2GetLine){

      //       }
      //       else if(diffEditor.getModifiedEditor().getModel().getFullModelRange().endLineNumber < step2GetLine){
      //         console.log(diffEditor.getModifiedEditor().getModel().getFullModelRange().endLineNumber)
      //         console.log(step2GetLine)
      //         for (let i = 1 ; i <=step2GetLine-diffEditor.getModifiedEditor().getModel().getFullModelRange().endLineNumber; i++ ){
      //           function lineFilter(element, index, array) {
      //             return element !== diffEditor.getModifiedEditor().getPosition().lineNumber+i;
      //           }
      //           myScodeSecLine = myScodeSecLine.filter(lineFilter)
      //         }
      //         myScodeSecLine = myScodeSecLine.map((value) => {
      //           if (value > diffEditor.getModifiedEditor().getPosition().lineNumber) return value - (step2GetLine-diffEditor.getModifiedEditor().getModel().getFullModelRange().endLineNumber);
      //           else return value;
      //         });
      //         step2GetLine = diffEditor.getModifiedEditor().getModel().getFullModelRange().endLineNumber;
      //       }


      //     if(e.code === "Delete" && diffEditor.getModifiedEditor().getPosition().column === test.getModifiedEditor().getModel().getLineMaxColumn(diffEditor.getModifiedEditor().getPosition().lineNumber)){
      //       console.log("마지막");
      //       myCondition2.set(true); 
      //     //   console.log(test.getModifiedEditor().getPosition().column);
      //     // console.log(test.getModifiedEditor().getModel().getLineMaxColumn(15))
      //     }
      //     else{
      //       myCondition2.set(false); 
      //     }


      //       if(e.code==="Enter" && myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getPosition().lineNumber) != -1){
      ...
      //           console.log(diffEditor.getModifiedEditor().getModel()._tokens._len)

      //           myScodeSecLine = myScodeSecLine.map((value) => {
      //             if (value >= diffEditor.getModifiedEditor().getPosition().lineNumber) return value + 1;
      //             else return value;
      //           });
      //           myScodeSecLine.push(diffEditor.getModifiedEditor().getPosition().lineNumber)
      //           console.log(myScodeSecLine)
      //           step2GetLine = diffEditor.getModifiedEditor().getModel().getFullModelRange().endLineNumber;
      //         }
      //         }, 10);
      ...
      //   //     if (myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getPosition().lineNumber) !== -1) {
      //   //   diffEditor.getModifiedEditor().updateOptions({ readOnly: false });
      //   // }
      //   // else {
      //   //   diffEditor.getModifiedEditor().updateOptions({ readOnly: true });
      ...
      //   if (myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getSelection().startLineNumber) != -1 && myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getSelection().endLineNumber) != -1) {
      //     diffEditor.getModifiedEditor().updateOptions({ readOnly: false });
      //         // 선택한 범위 사이가 readonly부분이 있으면 선택한 범위 한번에 변경못함
      //         for (let i = 0 ; i <=diffEditor.getModifiedEditor().getSelection().endLineNumber - diffEditor.getModifiedEditor().getSelection().startLineNumber; i++ ){
      //           if(myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getSelection().startLineNumber+i) === -1){
      //             diffEditor.getModifiedEditor().updateOptions({ readOnly: true }); break;
      //           }
      //         }
      ...
      //       if (diffEditor.getModifiedEditor().getPosition().column === 1 && myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getPosition().lineNumber) != -1) {
      //         if(myScodeSecLine.indexOf(diffEditor.getModifiedEditor().getPosition().lineNumber-1) === -1){
      ...
      //           if(e.code ==="Backspace"){
      //             function lineFilter(element, index, array) {
      //               return element !== diffEditor.getModifiedEditor().getPosition().lineNumber;
      //             }
      //           myScodeSecLine = myScodeSecLine.filter(lineFilter)
      //           myScodeSecLine = myScodeSecLine.map((value) => {
      //             if (value > diffEditor.getModifiedEditor().getPosition().lineNumber) return value - 1;
  ...
  handleResetStep2 = () => {
    //step2GetLine = monaco.editor.getModels()[1]._tokens._len;           _tokens._len ==  getFullModelRange() 둘이 총 라인수 나타냄
    ...
    /////////////////////////////////////////// 부분적인 readonly 기능 //////////////////////////////////////////
    //전체 라인 정보 test.getModifiedEditor().getModel().getFullModelRange()
  }
