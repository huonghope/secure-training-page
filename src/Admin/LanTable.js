...
export default function LanTable() {
  ...
  const [languages,setLanguages]=React.useState([
    // {
    //   languageId: '123kf-324',
    //   languageType:'Java',
    //   languageRegDate: '2020-07-08'
    // },
    // {
    //   languageId: '123kf-324',
    //   languageType:'C',
  ...
  const handleDelete = () => {
    ...
    const url = `http://${process.IP}:10000/admin/lan/${item.id}`;
      ...
      <Table className={classes.table} size="small"  aria-label="simple table" >
        <TableHead>
          <TableRow style={{backgroundColor:'#607d8b', }}>
            <TableCell style={{color:'white', fontWeight:'bold'}}>language type</TableCell>
        ...
        <TableBody>
          {languages.map((row,i) => (
            <TableRow key={row.languageId}>
              ...
              <TableCell align="left">{row.languageRegDate.substr(0, 10)}</TableCell>
              <TableCell align="center">
                <IconButton value={row.languageId} name={row.languageType} edge="end" aria-label="delete" style={{ color: '#607d8b' }} onClick={handleOpen}>
                  <DeleteIcon />
                </IconButton>
              </TableCell>
            </TableRow>
