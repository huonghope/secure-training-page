...
export default function FirCategoryTable() {
  ...
  const handleDelete = () => {
    ...
    const url = `http://${process.IP}:10000/admin/category/${item.id}`;
      ...
      <Table className={classes.table} size="small" aria-label="simple table">
        ...
        <TableBody>
          {categories.map((row,i) => (
            <TableRow key={row.scategoryId}>
              <TableCell component="th" scope="row">
                {row.scategoryName}
              </TableCell>
              <TableCell align="left">{row.scRegDate}</TableCell>
              <TableCell align="center">
                <IconButton value={row.scategoryId} name={row.scategoryName} edge="end" aria-label="delete" style={{ color: '#607d8b' }} onClick={handleOpen}>
                  <DeleteIcon />
                </IconButton>
              </TableCell>
            </TableRow>
