...
export default function Modify(props) {
    ...
    const handleSubmit = (e) => {
        ...
        // console.log(props.match.params.id, data.secName,data.languageId,data.badfile,data.badDescription,data.goodfile,data.goodDescription,data.lineNum,keywords.join('/'),data.seclineNum,0)
        ...
        form2.append('scodeId',props.match.params.id)
    ...
    //계속 걸림 체크해보기
    const checkNull = ()=>{
        if(data.secId === "" || data.languageId === "" || data.badDescription==="" || data.goodDescription==="" || data.lineNum==="" || keywords === "" || data.seclineNum === "") return true;
    ...
    useEffect(()=>{
        ...
        // const scodeId = '5ec3ca75-1110-4638-98fa-d11c7c2ae6b9';
        const url = `http://${process.IP}:10000/trainer/sb/${scodeId}`;
        const url2 = `http://${process.IP}:10000/lan`;
        const url3 = `http://${process.IP}:10000/admin/category`;
        const url4 = `http://${process.IP}:10000/trainer/sb/sv`;
        axios.get(url,{
            headers: {
        ...
        setData({...data,
            ...
            scategoryId: response.data.scategoryId,
            secId: response.data.secId,
            goodfileName:response.data.scodeSecFile, 
            badfileName:response.data.scodeVulFile,
            badDescription:response.data.scodeSecDesc,
            goodDescription:response.data.scodeVulDesc,
            lineNum:response.data.scodeLineNum,
            seclineNum:response.data.scodeSecLine})
        setKeywords(response.data.scodeKeyword.split('/'))
                            
        if(response.data.scodeApproval != 0){
            alert('이미 승인되었거나 거절된 문제는 수정할 수 없습니다!')
            props.history.goBack();
        }

        //소분류 목록 받아오기
        let form = new FormData();
        form.append('scategoryId',response.data.scategoryId);
       axios({
           method:'POST',
           url: `${url4}`,
           data: form,
